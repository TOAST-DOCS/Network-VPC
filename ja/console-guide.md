...
## Network > VPC > コンソール使用ガイド { #network-vpc-console-guide }
...
> [参考]
> 安定した運用のために、静的ルートはサブネットあたり最大20個まで作成できるように制限されています。
> 単一サブネット内で21個以上の静的ルートを作成する必要がある場合は、[カスタマーセンター](https://www.nhncloud.com/kr/support/inquiry)までお問い合わせください。
> 
> 最大作成数の制限を解除しても、DHCPプロトコルの制約により、作成可能な静的ルートの数は制限されます。
> * 静的ルート情報は、DHCPの「classless-static-routes」オプション(Option 121)を通じて伝達されます。
> * DHCPプロトコルは、各オプションの最大長を255バイトに制限しています。
> * ルートの宛先CIDRのサブネットマスク長(/8、/16、/24、/32など)によって、各ルートが占めるバイト数が変わり、追加できる数が変動します。
> * クラウドの運用に必要なシステム予約ルートが先に領域を占有するため、一般的にサブネットあたり最大20～30個程度のユーザー定義ルートの追加が可能です。
> * 「classless-static-routes」オプションの技術的な事項については、[RFC 3442](https://tools.ietf.org/html/rfc3442)をご参照ください。

<a id="subnet-create"></a>
### サブネットの作成 { #subnet-create }
...
### サブネットの修正 { #subnet-modify }
サブネットの名前を変更できます。 

<a id="subnet-delete"></a>
### サブネットの削除 { #subnet-delete }
選択したサブネットを削除します。サブネットの削除は、インスタンスやロードバランサーなど、サブネットにIPを割り当てられているリソースが含まれていない場合にのみ可能です。


<a id="routing-table-connect"></a>
### ルーティングテーブルの接続 { #routing-table-connect }

選択したサブネットを、選択したルーティングテーブルに「明示的」に関連付けます。

サブネットを作成すると、VPC のデフォルトルーティングテーブルに「暗黙的」に関連付けられます。ルーティングテーブル関連付け機能を使用して、デフォルトルーティングテーブルまたは他のルーティングテーブルに「明示的」に関連付けることができます。

<a id="routing-table-disconnect"></a>
### ルーティングテーブルの接続解除 { #routing-table-disconnect }

サブネットとルーティングテーブルが「明示的」に関連付けられている場合、関連付けを解除します。

ルーティングテーブルとの明示的な関連付けが解除されたサブネットは、VPC の **デフォルトルーティングテーブル**に再び「暗黙的」に関連付けられます。

> [注記]
> * 暗黙的な関連付け: サブネットが自動的に **デフォルトルーティングテーブル**に関連付けられた状態です。別のルーティングテーブルを VPC のデフォルトルーティングテーブルとして指定すると、既存のデフォルトルーティングテーブルに暗黙的に関連付けられていたサブネットは関連付けが解除され、新たに指定されたデフォルトルーティングテーブルに自動的に関連付けられます。
>   この場合、**サブネット基本情報 > ルーティングテーブル**の項目に **(デフォルト関連付けルーティングテーブル)** と表示されます。
> * 明示的な関連付け: **ルーティングテーブルの関連付け**機能を使用して、明示的にサブネットを指定したルーティングテーブルに関連付けた状態です。デフォルトルーティングテーブルに関連付けた場合を除き、すべて明示的な関連付けとなります。デフォルトルーティングテーブルを指定すると、暗黙的な関連付けの状態が明示的な関連付けの状態に変更されます。この場合、デフォルトルーティングテーブルを変更しても、サブネットは既存のルーティングテーブルとの関連付けを維持します。
