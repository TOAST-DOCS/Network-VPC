...
## Network > VPC > 콘솔 사용 가이드 { #network-vpc-console-guide }
...
> [참고]
> 안정적인 운영을 위해 정적 라우트는 서브넷당 최대 20개까지 생성할 수 있도록 제한되어 있습니다.
> 단일 서브넷 내에서 21개 이상의 정적 라우트 생성이 필요한 경우 [고객 센터](https://www.gov-nhncloud.com/kr/support/inquiry)로 문의해 주시기 바랍니다.
> 
> 최대 생성 개수 제한을 해제하더라도 DHCP 프로토콜의 제약으로 인해 생성 가능한 정적 라우트 개수가 제한됩니다.
> * 정적 라우트 정보는 DHCP의 'classless-static-routes' 옵션(Option 121)을 통해 전달됩니다.
> * DHCP 프로토콜은 각 옵션의 최대 길이를 255바이트로 제한하고 있습니다.
> * 라우트의 목적지 CIDR의 서브넷 마스크 길이(/8, /16, /24, /32 등)에 따라 각 라우트가 차지하는 바이트 수가 달라져 추가할 수 있는 개수가 변동됩니다.
> * 클라우드 운영에 필요한 시스템 예약 라우트들이 먼저 공간을 차지하여, 일반적으로 서브넷당 최대 20~30개 정도의 사용자 정의 라우트 추가가 가능합니다.
> * 'classless-static-routes' 옵션의 기술적인 사항은 [RFC 3442](https://tools.ietf.org/html/rfc3442)를 참고하시기 바랍니다.

<a id="subnet-create"></a>
### 서브넷 생성 { #subnet-create }
...
> 단일 VPC 내에서 11개 이상의 서브넷 생성이 필요한 경우 [고객 센터](https://www.gov-nhncloud.com/kr/support/inquiry)로 문의해 주시기 바랍니다. 

<a id="subnet-modify"></a>
### 서브넷 수정 { #subnet-modify }
서브넷의 이름을 변경할 수 있습니다. 

<a id="subnet-delete"></a>
### 서브넷 삭제 { #subnet-delete }
선택한 서브넷을 삭제합니다. 서브넷 삭제는 인스턴스나 로드 밸런서 등, 서브넷에서 IP를 할당받은 자원이 포함되어 있지 않은 경우에만 가능합니다. 

<a id="routing-table-connect"></a>
### 라우팅 테이블 연결 { #routing-table-connect }
...
### 라우팅 테이블 연결 해제 { #routing-table-disconnect }
